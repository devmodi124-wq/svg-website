...
if (problems.length > 0) {
  console.error(
    [
      ...
      "",
      "  Set them in the Cloudflare dashboard (Settings → Variables and Secrets)",
      "  for deploys, or in .env.local to build locally. Note that a variable",
      "  added after a build only takes effect on the NEXT build.",
      "",
    ].join("\n"),
  );
  process.exit(1);
